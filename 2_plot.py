...
# #     ds = xr.open_datatree("results/2_coh_boxcar.zarr", engine="zarr")[
# #         "polarization_vv"
# #     ].ds
...
# def main() -> None:
#     data = xr.open_datatree("results/2_coh_boxcar.zarr")
#     fig, axes = plt.subplots(
#         nrows=len(data),
...
#         coarse.transpose().power_no_coh.pipe(lambda x: np.abs(20 * np.log10(x))).plot(
#             ax=ax
#         )
#         ax.set_title(ds_name)
#     fig.savefig("results/2_coh_boxcar_power.png")
...
def main() -> None:
    ...
    for ax, (ds_name, ds) in zip(
        ...
        ds.dataset.transpose().power_no_coh.pipe(
